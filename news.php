<!DOCTYPE html>
<html>
<head>
    <meta charset="utf-8" />
    <title>Fresh Food store | Thực phẩm sạch</title>
    <link rel="shortcut icon" href="images/logo/favicon.ico" />
    <link href="Content/bootstrap.css" rel="stylesheet" />
    <link href="Content/font-awesome.css" rel="stylesheet" />
    <link href="Content/style.css" rel="stylesheet" />
</head>
<body>
    <header class="sticky-top">
        <nav id="main-menu" class="navbar navbar-expand-lg navbar-light bg-lightk">
            <div class="container">
                <div class="pull-left">
                    <a class="navbar-brand" href="index.php">
                        <img src="images/logo/logo.png" />
                    </a>
                </div>
                <div class="search">
                    <form class="search-box top-search" method="get" action="search.php">
                        <input class="form-control form-control-lg text-seach" type="text" placeholder="Tên sản phẩm bạn cần tìm..." name="search_query">
                        <button class="btn" type="submit" name="submit_search" value="Tìm kiếm">
                            <i class="fa fa-search" style="font-size: 22px"></i>
                        </button>
                    </form>
                </div>
                <div class="collapse navbar-collapse pull-right" id="navbarSupportedContent">
                    <ul class="navbar-nav ml-auto pull-right">
                        <li class="nav-item active">
                            <a class="nav-link" href="index.php">Trang chủ</a>
                        </li>
                        <li class="nav-item dropdown">
                            <div class="menu-product">
                                <a class="nav-link dropdown-toggle" href="#">
                                    Sản phẩm
                                </a>
                                <ul class='level0'>
                                    <li class="child">
                                        <a href="list-product.php">Rau củ quả<i class="fa fa-angle-right"></i></a>
                                        <ul class='level1 hidden-xs'>
                                            <li><a href="list-product.php">Trái cây tươi</a></li>
                                            <li><a href="#">Trái cây sấy</a></li>
                                            <li><a href="#">Rau xanh</a></li>
                                        </ul>
                                    </li>
                                    <li class="child">
                                        <a href="#">Thức uống<i class="fa fa-angle-right"></i></a>
                                        <ul class='level1 hidden-xs'>
                                            <li><a href="#">Sữa</a></li>
                                            <li><a href="#">Nước ép</a></li>
                                        </ul>
                                    </li>
                                    <li class="child">
                                        <a href="#">Chiết xuất</a>
                                    </li>
                                    <li class="child">
                                        <a href="#">Các loại hạt<i class="fa fa-angle-right"></i></a>
                                        <ul class='level1 hidden-xs'>
                                            <li><a href="#">Gạo nếp</a></li>
                                            <li><a href="#">Đậu</a></li>
                                            <li><a href="#">Loại khác</a></li>
                                        </ul>
                                    </li>
                                </ul>
                            </div>
                        </li>
                        <li class="nav-item">
                            <a class="nav-link" href="news.php">Bài viết</a>
                        </li>
                        <li class="nav-item">
                            <a class="nav-link" href="contact.php">Liên hệ</a>
                        </li>
                        <li class="nav-item icon">
                            <div class="menu-product">
                                <a class="nav-link" href="#">
                                    <i class="fa fa-user-circle"></i>
                                </a>
                                <ul>
                                    <li>
                                        <a class="w-75" href="login.php">Đăng nhập</a>
                                    </li>
                                    <li>
                                        <a class="w-75" href="login.php">Đăng ký</a>
                                    </li>
                                </ul>
                            </div>
                        </li>
                        <li class="nav-item icon">
                            <a class="nav-link" href="cart.php">
                                <i class="fa fa-shopping-cart"></i>
                            </a>
                        </li>
                    </ul>
                </div>
            </div>
        </nav>
    </header>

    <main>
        <div class="list-news container">
            <nav aria-label="breadcrumb">
                <ol class="breadcrumb">
                    <li class="breadcrumb-item"><a href="index.php">Trang chủ</a></li>
                    <li class="breadcrumb-item active" aria-current="page">Bài viết</li>
                </ol>
            </nav>
            <div class="the-title">
                <h2>
                    Bài viết
                </h2>
                <img class="img-fluid" src="images/hinh-anh-trang-chu/logo-leaf.png" />
            </div>
            <div class="news-item">
                <ul>
                    <li class="news">
                        <img src="images/news/trong-rau-trong-thung-xop.jpg" />
                        <div class="title">
                            <h3>
                                Mẹo trồng rau trong thùng xốp
                            </h3>
                        </div>
                        <div class="time">
                            <small>25/03/2020</small>
                        </div>
                        <div class="content">
                            <p>
                                Phương thức trồng rau trong thùng xốp ngày càng được ưa chuộng bởi sự đơn giản, tiện lợi, dễ trồng và nhanh thu hoạch. Vậy làm thế nào để trồng rau trong hộp xốp thành công?
                            </p>
                            <a href="https://www.facebook.com/permalink.php?story_fbid=116127186688774&id=113487710286055&__tn__=K-R" target="_blank">
                                Đọc tiếp »
                            </a>
                        </div>
                    </li>

                    <li class="news">
                        <img src="images/news/94871-axit-beo-omega-3.jpg" />
                        <div class="title">
                            <h3>
                                Mẹo trồng rau trong thùng xốp
                                Mẹo trồng rau trong thùng xốp
                                Mẹo trồng rau trong thùng xốp
                            </h3>
                        </div>
                        <div class="time">
                            <small>25/03/2020</small>
                        </div>
                        <div class="content">
                            <p>
                                Phương thức trồng rau trong thùng xốp ngày càng được ưa chuộng bởi sự đơn giản, tiện lợi, dễ trồng và nhanh thu hoạch. Vậy làm thế nào để trồng rau trong hộp xốp thành công?
                                Phương thức trồng rau trong thùng xốp ngày càng được ưa chuộng bởi sự đơn giản, tiện lợi, dễ trồng và nhanh thu hoạch. Vậy làm thế nào để trồng rau trong hộp xốp thành công?
                                Phương thức trồng rau trong thùng xốp ngày càng được ưa chuộng bởi sự đơn giản, tiện lợi, dễ trồng và nhanh thu hoạch. Vậy làm thế nào để trồng rau trong hộp xốp thành công?
                                Phương thức trồng rau trong thùng xốp ngày càng được ưa chuộng bởi sự đơn giản, tiện lợi, dễ trồng và nhanh thu hoạch. Vậy làm thế nào để trồng rau trong hộp xốp thành công?
                                Phương thức trồng rau trong thùng xốp ngày càng được ưa chuộng bởi sự đơn giản, tiện lợi, dễ trồng và nhanh thu hoạch. Vậy làm thế nào để trồng rau trong hộp xốp thành công?
                            </p>
                            <a href="https://www.facebook.com/permalink.php?story_fbid=116127186688774&id=113487710286055&__tn__=K-R" target="_blank">
                                Đọc tiếp »
                            </a>
                        </div>
                    </li>

                    <li class="news">
                        <img src="images/news/cach-bao-quan-rau-cu-qua-tuoi-ngon-khong-mat-chat-dinh-duong-1_result.jpg" />
                        <div class="title">
                            <h3>
                                Mẹo trồng rau trong thùng xốp
                            </h3>
                        </div>
                        <div class="time">
                            <small>25/03/2020</small>
                        </div>
                        <div class="content">
                            <p>
                                Phương thức trồng rau trong thùng xốp ngày càng được ưa chuộng bởi sự đơn giản, tiện lợi, dễ trồng và nhanh thu hoạch. Vậy làm thế nào để trồng rau trong hộp xốp thành công?
                            </p>
                            <a href="https://www.facebook.com/permalink.php?story_fbid=116127186688774&id=113487710286055&__tn__=K-R" target="_blank">
                                Đọc tiếp »
                            </a>
                        </div>
                    </li>

                    <li class="news">
                        <img src="images/news/unnamed.png" />
                        <div class="title">
                            <h3>
                                Mẹo trồng rau trong thùng xốp
                            </h3>
                        </div>
                        <div class="time">
                            <small>25/03/2020</small>
                        </div>
                        <div class="content">
                            <p>
                                Phương thức trồng rau trong thùng xốp ngày càng được ưa chuộng bởi sự đơn giản, tiện lợi, dễ trồng và nhanh thu hoạch. Vậy làm thế nào để trồng rau trong hộp xốp thành công?
                            </p>
                            <a href="https://www.facebook.com/permalink.php?story_fbid=116127186688774&id=113487710286055&__tn__=K-R" target="_blank">
                                Đọc tiếp »
                            </a>
                        </div>
                    </li>
                </ul>
            </div>
        </div>
    </main>

    <footer>
        <div class="container">
            <div class="leaf-free">
                <img src="images/hinh-anh-trang-chu/leaf-free-img.png" />
            </div>
            <div class="row">
                <div class="col-4">
                    <h3>
                        Thông tin liên hệ
                    </h3>
                    <div class="address">
                        Địa chỉ: Lầu 8, 79 Trương Định, Quận 1, Hồ Chí Minh, Việt Nam<br>
                        Điện thoại: 0975103431<br>
                        Hotline: 0975103431<br>
                        Email: <a href="#">truongquochung204@gmail.com</a><br>
                    </div>
                </div>
                <div class="col-4">
                    <h3>Về chúng tôi</h3>
                    <p>Hiệp Thực Phẩm Minh Bạch (FTA) được thành lập tại Việt Nam vào năm 2016, với mục tiêu cụ thể là phát triển và thúc đẩy các tiêu chuẩn minh bạch và an toàn thực phẩm từ trang trại đến bàn ăn.</p>
                    <div class="pre-footer-subscribe-box pre-footer-subscribe-box-vertical">
                        <h3>Nhận bản tin</h3>
                        <p>Hãy để lại Email của bạn để nhận được những thông tin mới nhất được cập nhật liên tục !</p>
                        <div class="input-group">
                            <input type="text" id="txtSubscribe" class="form-control" placeholder="youremail@mail.com">
                            <span class="input-group-btn">
                                <button id="btnSubscribe" class="btn btn-info">Subscribe</button>
                            </span>
                        </div>
                    </div>
                </div>
                <div class="col-4">
                    <div id="fb-root"></div>
                    <script>
                        (function (d, s, id) {
                            var js, fjs = d.getElementsByTagName(s)[0];
                            if (d.getElementById(id)) return;
                            js = d.createElement(s); js.id = id;
                            js.src = 'https://connect.facebook.net/vi_VN/sdk.js#xfbml=1&version=v3.1';
                            fjs.parentNode.insertBefore(js, fjs);
                        }(document, 'script', 'facebook-jssdk'));</script>

                    <div class="fb-page"
                         data-href="https://www.facebook.com/FreshFood-Store-Th%E1%BB%B1c-Ph%E1%BA%A9m-S%E1%BA%A1ch-Cho-M%E1%BB%8Di-Nh%C3%A0-113487710286055/"
                         data-small-header="false"
                         data-adapt-container-width="true"
                         data-hide-cover="false"
                         data-show-facepile="true">
                    </div>
                </div>
            </div>
        </div>
        <div class="footer">
            <div class="container">
                <div class="row">
                    <div class="col-8">
                        2020 © Association of Food Transparency. ALL Rights Reserved.
                        <a href="#">Chính Sách Bảo Mật</a> |
                        <a href="#">Điều Khoản Dịch Vụ</a>
                    </div>
                    <div class="col-4">
                        <div class="pull-right">
                            <i class="fa fa-facebook"></i>
                            <i class="fa fa-google"></i>
                            <i class="fa fa-youtube"></i>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </footer>
    <a href="javascript:void(0);" class="back-to-top" style="display: block;"><span>Top</span></a>
    <div id="script">
        <script src="Scripts/popper.js"></script>
        <script src="Scripts/jquery-3.0.0.js"></script>
        <script src="Scripts/bootstrap.js"></script>
        <script src="Scripts/script.js"></script>
    </div>
</body>
</html>